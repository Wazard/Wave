...
implements Renderable {
    ...
    public void setFilter(int f) throws SlickException {
        ...
        try {
            ...
            if (this.transparent != null) {
                trans = new int[]{(int)(this.transparent.r * 255.0f), (int)(this.transparent.g * 255.0f), (int)(this.transparent.b * 255.0f)};
            }
            this.texture = InternalTextureLoader.get().getTexture(this.ref, this.flipped, this.filter, trans);
        }
        catch (IOException e) {
            Log.error(e);
    ...
    public Image(int width, int height) throws SlickException {
        ...
        try {
            this.texture = InternalTextureLoader.get().createTexture(width, height);
        }
        catch (IOException e) {
            Log.error(e);
    ...
    public void drawFlash(float x, float y, float width, float height, Color col) {
        ...
        if (GLContext.getCapabilities().GL_EXT_secondary_color) {
            GL.glEnable(33880);
            EXTSecondaryColor.glSecondaryColor3ubEXT((byte)(col.r * 255.0f), (byte)(col.g * 255.0f), (byte)(col.b * 255.0f));
        }
