...
public class WitchBoss extends GameObject {
   private Handler handler;
   Random r = new Random();
   private HUD hud;
   private float life = 160.0F;
   private int i = 0;
   private int a;
   private int x1;
   private int y1;
   ...
   public WitchBoss(int x, int y, ID id, Handler handler, HUD hud) {
      ...
      this.image = ss.grabBossImage(1, 3, 55, 72);
      this.velX = 0.0F;
      this.velY = 2.0F;
   }
   ...
   public void tick() {
      ...
      if (this.timer2 <= 0) {
         if (this.times[0] <= 0) {
            this.x = (float)(Game.WIDTH / 4 - 36);
            this.handler.clearSmallEnemies();
            this.y = -140.0F;
            this.a = this.r.nextInt(3);
            this.times[0] = 1;
         } else if (this.times[0] == 1 && this.height) {
            this.x = (float)(Game.WIDTH / 2 - 36);
            this.y = -140.0F;
            this.times[0] = 2;
         } else if (this.times[0] == 2 && this.height) {
            this.x = (float)(Game.WIDTH * 3 / 4 - 36);
            this.y = -140.0F;
            this.times[0] = 3;
         }

         if (this.y >= (float)Game.HEIGHT) {
            this.height = true;
         } else if (this.y < (float)(Game.HEIGHT / 6 - 5)) {
            this.height = false;
         }
         ...
         if (this.times[0] == 3 && this.height) {
            this.timer = 1;
            if (this.r.nextInt(2) == 0 && !this.control) {
               this.x = (float)(Game.WIDTH / 6);
            } else if (!this.control) {
               this.x = (float)(Game.WIDTH * 5 / 6);
            }

            this.control = true;
            this.y = (float)(Game.HEIGHT / 6);
            if (this.i <= 10) {
               this.velY = 0.0F;
               if (this.timer3 >= 0) {
                  --this.timer3;
         ...
         if (this.life <= 0.0F) {
            ...
            this.hud.score(this.hud.getScore() + 2000);
   ...
   private void spawn() {
      ...
      if (this.a == 0) {
         do {
            this.handler.addObject(new MagicBullet((float)this.x1, -15.0F, ID.MagicEnemy, this.handler, 0));
            this.x1 += 80;
         ...
         do {
            this.handler.addObject(new MagicBullet(-15.0F, (float)this.y1, ID.MagicEnemy, this.handler, 1));
            this.y1 += 80;
